			...
			<?php else : ?>
				...
				<div class="s-content" style="margin-top: 35px">
					<div class="content-block">
						<p class="text-attention"><?= ($data['name'] !== 'Каталог') ? $data['name'] : $this->set['keywords'] ?> в Донецке, Макеевке, Харцызске, ДНР есть в нашем магазине по адресу: <?= $this->set['address'] ?>. Сделать заказ можно по тел. <?= $this->set['phone'] ?>. Цены доступные. В ассортименте есть <?= ($data['name'] !== 'Каталог') ? $data['name'] : $this->set['keywords'] ?> от известных производителей. Есть возможность доставки по Донецку, Макеевке, Харцызску, ДНР</p>
